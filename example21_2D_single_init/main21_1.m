...
pde = ex21_2_MPFCdata(para);

%% Run:

time = struct('T',T,'t0',t0,'dt',dtmin,'tsave',tsave,...
              'dtmin',dtmin,'dtmax',dtmax,'dtalpha',dtalpha,'dttol',dttol);
MPFC_2D_CN_SAV_adaptive_time(pde,domain,N,N,time,option); %% ok

figure(1);
% subplot(1,2,1)
energy=load([pde.name,'e',num2str(pde.epsilon),'M',num2str(pde.M),'b_bar',num2str(pde.beta_bar),'b',num2str(pde.beta),'dt',num2str(dtmin),'_energy.txt']);
